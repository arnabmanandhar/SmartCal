...
#if 'val_' not in col
non_val_loss_columns = [col for col in df.columns if 'val_' not in col]
...
for i, column in enumerate(non_val_loss_columns):
    if column != 'epoch':  # Skip 'epoch' column
        ax.plot(df.index, df[column], label=column, linestyle=line_styles[i])
